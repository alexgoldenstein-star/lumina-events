  ...
  return (
    <div className="min-h-screen bg-nude-50 flex">
      ...
      <div className="hidden lg:block w-1/2 relative">
        <img src="https://images.unsplash.com/photo-1519741497674-611481863552?w=900&q=85&fit=crop"
          alt="" className="absolute inset-0 w-full h-full object-cover"/>
        <div className="absolute inset-0 bg-black/40"/>
        <div className="absolute inset-0 flex items-center justify-center">
          <LogoJR size="lg" light/>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          ...
          <h1 className="text-2xl font-light text-ink-900 mb-1" style={{fontFamily:'Georgia, serif'}}>Bienvenida</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[11px] tracking-widest uppercase text-ink-400 mb-2">Email</label>
            ...
            <div>
              <label className="block text-[11px] tracking-widest uppercase text-ink-400 mb-2">Contraseña</label>
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3.5 bg-ink-900 text-white text-[11px] tracking-widest uppercase hover:bg-ink-700 transition-colors disabled:opacity-50 mt-2">
              {loading?'Ingresando...':'Ingresar'}
            </button>
          </form>

          <div className="mt-6 text-center">
            <Link to="/" className="text-[11px] text-ink-400 hover:text-ink-700 tracking-widest uppercase transition-colors">
              ← Volver al sitio
            </Link>
          </div>
        </div>
...
export function Register() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true); setError('')
    try {
      await register(form.email.trim().toLowerCase(), form.password, { nombre:form.nombre })
      navigate('/dashboard')
    } catch(err) {
  ...
  return (
    <div className="min-h-screen bg-nude-50 flex items-center justify-center p-8">
      <div className="w-full max-w-sm">
        <div className="mb-10 flex justify-center"><LogoJR size="md"/></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[11px] tracking-widest uppercase text-ink-400 mb-2">Tu nombre</label>
