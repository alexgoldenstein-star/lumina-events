...
function PrivateRoute({ children, permiso }) {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-nude-50"><Spinner size="lg"/></div>
...
function EventoEditPage() {
  ...
  useEffect(() => {
    ...
    const ownerUid = teamOwner || user.uid
    get(eventoRef(ownerUid, id)).then(snap => {
      if (snap.exists()) setEvento(snap.val())
      else get(eventoRef(user.uid, id)).then(s2 => {
        if (s2.exists()) setEvento(s2.val())
    ...
    <AuthProvider>
      <BrowserRouter>
        <Routes>
          ...
          {/* App — orden importa: específicas primero */}
          <Route path="/dashboard"          element={<PrivateRoute><Dashboard/></PrivateRoute>}/>
          <Route path="/eventos/nuevo"      element={<PrivateRoute permiso="crearEventos"><EventoForm/></PrivateRoute>}/>
          <Route path="/eventos/:id/editar" element={<PrivateRoute permiso="crearEventos"><EventoEditPage/></PrivateRoute>}/>
          ...
          <Route path="/exportar"           element={<PrivateRoute permiso="gestionarUsuarios"><ExportarDatos/></PrivateRoute>}/>
